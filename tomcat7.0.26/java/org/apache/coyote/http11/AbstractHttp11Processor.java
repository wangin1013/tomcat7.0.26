...
public abstract class AbstractHttp11Processor<S> extends AbstractProcessor<S> {
	...
	private boolean useCompression() {
		...
		if (noCompressionUserAgents != null) {
			...
			if (userAgentValueMB != null) {
				...
				if (noCompressionUserAgents != null && noCompressionUserAgents.matcher(userAgentValue).matches()) {
					return false;
				}
			}
		}
	...
	protected boolean statusDropsConnection(int status) {
		return status == 400 /* SC_BAD_REQUEST */|| status == 408 /* SC_REQUEST_TIMEOUT */|| status == 411 /* SC_LENGTH_REQUIRED */
				|| status == 413 /* SC_REQUEST_ENTITY_TOO_LARGE */|| status == 414 /* SC_REQUEST_URI_TOO_LONG */
				|| status == 500 /* SC_INTERNAL_SERVER_ERROR */|| status == 503 /* SC_SERVICE_UNAVAILABLE */
		...
		while (!error && keepAlive && !comet && !isAsync() && !endpoint.isPaused()) {
			...
			if (!error) {
				try {
					...
					getLog().info("调用adapter进行处理request及response：service方法");
	...
	protected void prepareRequest() {
		...
		if (transferEncodingValueMB != null) {
			...
			if (!addInputFilter(inputFilters, encodingName)) {
				...
				if (getLog().isDebugEnabled()) {
					getLog().debug(
							sm.getString("http11processor.request.prepare") + " Unsupported transfer encoding \""
									+ encodingName + "\"");
				}
				response.setStatus(501);
			}
		...
		if (http11 && (valueMB == null)) {
			...
			// 400 - Bad request
			if (getLog().isDebugEnabled()) {
				getLog().debug(sm.getString("http11processor.request.prepare") + " host header missing");
			}
			response.setStatus(400);
		}
	...
	private void prepareResponse() {
		...
		if (http09 == true) {
			// HTTP/0.9
			getOutputBuffer().addActiveFilter(outputFilters[Constants.IDENTITY_FILTER]);
			return;
		}
