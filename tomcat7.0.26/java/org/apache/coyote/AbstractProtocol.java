...
public abstract class AbstractProtocol implements ProtocolHandler, MBeanRegistration {
	...
	/**
	 * Unique ID for this connector. Only used if the connector is configured to
	 * use a random port as the port will change if stop(), start() is called.
	 */
	...
	protected abstract static class AbstractConnectionHandler<S, P extends AbstractProcessor<S>> implements
		...
		protected ConcurrentHashMap<S, P> connections = new ConcurrentHashMap<S, P>();
		...
		public SocketState process(SocketWrapper<S> socket, SocketStatus status) {
			...
			if (getLog().isDebugEnabled()) {
				getLog().debug(
						"process() entry " + "Socket: [" + logHashcode(socket.getSocket()) + "], " + "Processor ["
								+ logHashcode(processor) + "]");
			}
			...
			try {
				...
				if (getLog().isDebugEnabled()) {
					getLog().debug(
							"process() gotProcessor " + "Socket: [" + logHashcode(socket.getSocket()) + "], "
				...
				do {
					if (processor.isAsync() || state == SocketState.ASYNC_END) {
						...
						if (getLog().isDebugEnabled()) {
							getLog().debug(
									"process() asyncDispatch " + "Socket: [" + logHashcode(socket.getSocket()) + "], "
											+ "Processor: [" + logHashcode(processor) + "], " + "State: ["
											+ state.toString() + "]");
						}
						...
						if (getLog().isDebugEnabled()) {
							getLog().debug(
									"process() event " + "Socket: [" + logHashcode(socket.getSocket()) + "], "
											+ "Processor: [" + logHashcode(processor) + "], " + "State: ["
											+ state.toString() + "]");
						}
					} else {
						getLog().info("使用自定义处理器处理socket");
						state = processor.process(socket);
						if (getLog().isDebugEnabled()) {
							getLog().debug(
									"process() process " + "Socket: [" + logHashcode(socket.getSocket()) + "], "
											+ "Processor: [" + logHashcode(processor) + "], " + "State: ["
											+ state.toString() + "]");
						}
					}
					...
					if (state != SocketState.CLOSED && processor.isAsync()) {
						...
						if (getLog().isDebugEnabled()) {
							getLog().debug(
									"process() asyncPostProcess " + "Socket: [" + logHashcode(socket.getSocket())
											+ "], " + "Processor: [" + logHashcode(processor) + "], " + "State: ["
											+ state.toString() + "]");
						}
		...
		protected abstract void release(SocketWrapper<S> socket, P processor, boolean socketClosing, boolean addToPoller);
				...
				synchronized (this) {
					try {
						...
						ObjectName rpName = new ObjectName(getProtocol().getDomain() + ":type=RequestProcessor,worker="
								+ getProtocol().getName() + ",name=" + getProtocol().getProtocolName() + "Request"
								+ count);
						if (getLog().isDebugEnabled()) {
	...
	protected static class RecycledProcessors<P extends AbstractProcessor<S>, S> extends ConcurrentLinkedQueue<P> {
