...
/**
 * Server接口的标准实现,部署启动Catalian是非常有用的，但非必须的
 * 
...
public final class StandardServer extends LifecycleMBeanBase implements Server {
	...
	public StandardServer() {
		...
		log.info("执行StandardServer的构造函数！");
	...
	public void addService(Service service) {
		...
		synchronized (services) {
			...
			if (getState().isAvailable()) {
				try {
					log.info("添加StandardService对象，并执行service的start方法！");
					service.start();
	...
	public void await() {
		...
		log.info("使Catalina处于线程等待中，直到收到一个合适的关闭命令！这种方式保证了主线程，处于存活状态。");
	...
	/**
	 * Write the configuration information for <code>Context</code> out to the
	...
	protected void initInternal() throws LifecycleException {
		...
		log.info("执行StandardServer的initInternal方法，完成server的初始化！");
	...
	/**
	 * Obtain the MBean domain for this server. The domain is obtained using the
