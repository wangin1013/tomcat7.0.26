 ...
 * happens to be the context class loader at the time. The work-around is to
 * initialise these singletons when Tomcat's common class loader is the context
 * class loader.
 * <p>
...
public class JreMemoryLeakPreventionListener implements LifecycleListener {
	...
	/**
	 * Protect against the memory leak caused when the first call to
	 * <code>java.awt.Toolkit.getDefaultToolkit()</code> is triggered by a web
	 * application. Defaults to <code>false</code> because a new Thread is
	 * launched.
	 */
	...
	/**
	 * Protect against the memory leak, when the initialization of the Java
	 * Cryptography Architecture is triggered by initializing a MessageDigest
	 * during web application deployment. This will occasionally start a Token
	 * Poller thread with the thread's context class loader equal to the web
	 * application class loader. Instead we initialize JCA early. Defaults to
	 * <code>true</code>.
	 */
	 ...
	 * {@link URLConnection}s, regardless of type. Defaults to <code>true</code>
	 * .
	 */
	 ...
	 * identify any GC roots related to this. <a
	 * href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=6916498">
	 ...
	 * that there may be a web application class loader leak if the web app is
	...
	/**
	 * The first access to {@link DriverManager} will trigger the loading of all
	 * {@link java.sql.Driver}s in the the current class loader. The web
	 ...
	 * initialize during the startup of this Listener. This allows to pre-load
	 * classes that are known to provoke classloader leaks if they are loaded
	...
	public void lifecycleEvent(LifecycleEvent event) {
		...
		if (Lifecycle.BEFORE_INIT_EVENT.equals(event.getType())) {
			...
			try {
				 ...
				 * due to eventual calls to getAppContext() are: - Google Web
				 * Toolkit via its use of javax.imageio - Tomcat via its use of
				 * java.beans.Introspector.flushCaches() in 1.6.0_15 onwards -
				 * others TBD
				 */
				 ...
				 * a static reference to the context class loader.
				 */
				 ...
				 * include: - log4j versions 1.2.15 and earlier -
