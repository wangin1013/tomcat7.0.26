...
/**
 * Standard implementation of the <code>Service</code> interface. The associated
 * Container is generally an instance of Engine, but this is not required.
 * 
...
public class StandardService extends LifecycleMBeanBase implements Service {
	...
	protected void startInternal() throws LifecycleException {
		...
		// 第二步：启动内部定义的Connector（HTTP Connector，AJP Connector）
		synchronized (connectors) {
	...
	protected void initInternal() throws LifecycleException {
		...
		//实例化定义的Connector对象：
		//Http理解器：<Connector port="8080" protocol="HTTP/1.1" connectionTimeout="20000" redirectPort="8443" />
		//AJP连接器：<Connector port="8009" protocol="AJP/1.3" redirectPort="8443" />
		synchronized (connectors) {
			for (Connector connector : connectors) {
				try {
					connector.init();
